...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        for row in range(cell[0] - 1, cell[0] + 2):
            for column in range(cell[1] - 1, cell[1] + 2):
                ...
                if 0 <= row < self.height and 0 <= column < self.width:
                    if (row, column) not in self.safes and (row, column) not in self.mines:
        ...
        for sentence in self.knowledge:
            if newSentence != sentence and newSentence.cells.issubset(sentence.cells) and sentence.count > 0 and newSentence.count > 0:
                self.knowledge.append(Sentence(list(sentence.cells.difference(
                    newSentence.cells)), sentence.count - newSentence.count))
    ...
    def make_safe_move(self):
        ...
        """
        safeMoves = list(self.safes.difference(self.moves_made))

        if len(safeMoves) == 0:
            return None
        else:
            safeMov = random.choice(safeMoves)
            self.moves_made.add(safeMov)
            return safeMov
        #raise NotImplementedError
        ...
        for row in range(self.height):
            for column in range(self.width):
                if (row, column) not in self.moves_made and (row, column) not in self.mines:
                    randomMoves.append((row, column))

        if len(randomMoves) == 0:
            return None
        else:
            randMov = random.choice(randomMoves)
            self.moves_made.add(randMov)
            return randMov
        #raise NotImplementedError
